...
class _TopBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // MVVM: estado desde AuthViewModel (NO FirebaseAuth aquí)
    final authVM = context.watch<AuthViewModel>();
    final bool isLoggedIn = authVM.isLoggedIn;
    final String email = authVM.email ?? "";
        ...
        children: [
          ...
          if (isWide) ...[
            _NavLink(
              ...
              onTap: () => onShowInfo(
                "¿Qué es BookLoop?",
                "Es una plataforma exclusiva para la comunidad UNIMET de préstamo y circulación de material académico. "
                "Nuestra plataforma busca conectar a estudiantes y profesores dentro de un entorno seguro, facilitando "
                "el acceso equitativo a recursos educativos mediante la reutilización organizada.",
              ),
            ),
            _NavLink(
              'Misión',
              onTap: () => onShowInfo(
                "Nuestra Misión",
                "Proporcionar a la comunidad docente y estudiantil de la Universidad Metropolitana una plataforma centralizada, "
                "segura y exclusiva, que facilite el préstamo y circulación de material académico, fomentando la reutilización "
                "de recursos y el acceso equitativo a las herramientas de aprendizaje.",
              ),
            ),
            _NavLink(
              'Visión',
              onTap: () => onShowInfo(
                "Nuestra Visión",
                "Ser la plataforma de referencia para la comunidad de la Universidad Metropolitana en la reutilización organizada "
                "de material académico, reduciendo la dependencia de canales informales y la incertidumbre al momento de conseguir recursos.",
              ),
            ),
            const SizedBox(width: 12),
          ],
      ...
      child: const Column(
        ...
        children: [
          ...
          _BenefitItem(
            ...
            subtitle: 'Gestiona tus libros con miembros verificados.',
          ),
          SizedBox(height: 20),
          _BenefitItem(
            icon: Icons.history_edu,
            title: 'Acceso Equitativo',
            subtitle: 'Encuentra material sin depender de terceros.',
          ),
          SizedBox(height: 20),
          _BenefitItem(
            icon: Icons.volunteer_activism,
            title: 'Cultura Responsable',
            subtitle: 'Fomenta la reutilización entre unimetanos.',
          ),
        ],
      ),
    );
...
class _BenefitItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Icon(icon, color: StartPage.unimetOrange, size: 28),
        const SizedBox(width: 12),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
        ...
        children: [
          Image.asset(
            ...
            errorBuilder: (context, error, stackTrace) =>
                const Icon(Icons.menu_book, size: 80, color: Color(0xFF1B3A57)),
          ),
          const SizedBox(height: 14),
          Text(
            !isLoggedIn ? 'Tu acceso comienza aquí' : '¡Bienvenido de nuevo!',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: Color(0xFF1B3A57)),
          ),
          ...
          if (!isLoggedIn) ...[
            ...
            OutlinedButton(
              ...
              style: OutlinedButton.styleFrom(
                minimumSize: const Size(double.infinity, 50),
                side: const BorderSide(color: Color(0xFF1B3A57)),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
              ),
