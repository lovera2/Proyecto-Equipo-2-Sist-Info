...
class AdminMaterialManagementPage extends StatefulWidget {
  ...
  @override
  State<AdminMaterialManagementPage> createState() => _AdminMaterialManagementPageState();
}

class _AdminMaterialManagementPageState extends State<AdminMaterialManagementPage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => context.read<AdminMaterialViewModel>().cargarMateriales());
  }
  ...
  Widget _buildSafeImage(String? source, {double width = 40, double height = 60}) {
    ...
    try {
      if (source.startsWith('http://') || source.startsWith('https://')) {
        return Image.network(
          source, width: width, height: height, fit: BoxFit.cover,
          errorBuilder: (_, __, ___) => Icon(Icons.broken_image, size: width * 0.8, color: Colors.red),
        );
      }
      ...
      return Image.memory(
        ...
        errorBuilder: (context, error, stackTrace) => 
            Icon(Icons.broken_image, size: width * 0.8, color: Colors.red),
      );
    } catch (e) {
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: const [
                      ...
                      SizedBox(height: 6),
                      Text(
                        'Administra el catálogo de libros, consulta su información y controla la disponibilidad.',
                              ...
                              return Column(
                                children: [
                                  ...
                                  if (vm.materialSeleccionado != null) 
                                    Expanded(flex: 6, child: _buildDetailPanel(vm, context)),
                                ],
                              );
                            }
                            return Row(
                                ...
                                Expanded(
                                  ...
                                  child: vm.materialSeleccionado == null
                                      ? Container(
                                          ...
                                          child: const Center(
                                            child: Text("Selecciona un libro para ver sus detalles", style: TextStyle(color: Colors.grey)),
                                          ),
                                        )
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      title: Text(libro['title'] ?? 'Sin título', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                      subtitle: Text(libro['author'] ?? 'Autor desconocido', style: const TextStyle(fontSize: 12)),
  ...
  Widget _buildDetailPanel(AdminMaterialViewModel vm, BuildContext context) {
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                ClipRRect(
                  ...
                  child: _buildSafeImage(libro['imageUrl'], width: 100, height: 150),
                ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(libro['title'] ?? 'Sin Título', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: unimetBlue)),
                      const SizedBox(height: 4),
                      Text("Autor: ${libro['author']}", style: const TextStyle(fontSize: 16, color: Colors.black54)),
            ...
            const Text("Información General", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: unimetBlue)),
            const SizedBox(height: 12),
            ...
            const SizedBox(height: 8),
            _infoRow("Condición/Estado", libro['condition'] ?? 'No especificada'),
            const SizedBox(height: 20),
            const Text("Descripción", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: unimetBlue)),
            const SizedBox(height: 8),
            Text(libro['description'] ?? 'Sin descripción disponible', style: const TextStyle(color: Colors.black87, height: 1.5)),
  ...
  Widget _statusChip(String status) {
    ...
    final Color colorBg = esDisponible ? Colors.green.shade50 : Colors.red.shade50;
    final Color colorTxt = esDisponible ? Colors.green.shade700 : Colors.red.shade700;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
      decoration: BoxDecoration(color: colorBg, borderRadius: BorderRadius.circular(20)),
      child: Text(status.toUpperCase(), style: TextStyle(color: colorTxt, fontWeight: FontWeight.bold, fontSize: 12)),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Text(value ?? 'N/A', style: const TextStyle(fontWeight: FontWeight.bold, color: unimetBlue))),
        ],
      ),
    );
  }

  void _mostrarDialogoEdicion(BuildContext context, AdminMaterialViewModel vm, Map<String, dynamic> libro) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: const Text("Editar Material", style: TextStyle(color: unimetBlue, fontWeight: FontWeight.bold)),
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                TextField(controller: authorCtrl, decoration: const InputDecoration(labelText: 'Autor', border: OutlineInputBorder())),
                const SizedBox(height: 12),
                TextField(controller: categoryCtrl, decoration: const InputDecoration(labelText: 'Categoría', border: OutlineInputBorder())),
                const SizedBox(height: 12),
                TextField(controller: subjectCtrl, decoration: const InputDecoration(labelText: 'Materia', border: OutlineInputBorder())),
                const SizedBox(height: 12),
                TextField(controller: statusCtrl, decoration: const InputDecoration(labelText: 'Estado', border: OutlineInputBorder())),
              ],
            ),
          ),
          actions: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: unimetBlue, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
              onPressed: () async {
                if(titleCtrl.text.trim().isEmpty || authorCtrl.text.trim().isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("El título y el autor no pueden estar vacíos"), backgroundColor: Colors.red));
                  return;
                }
                ...
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("Material actualizado exitosamente"), backgroundColor: Colors.green)
                  );
                } catch (e) {
                  ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _confirmarEliminacion(BuildContext context, AdminMaterialViewModel vm) async {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
        title: const Text("Eliminar material", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
        content: const Text("Esta acción no se puede deshacer. Solo se permiten borrar libros 'disponibles'."),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              try {
                ...
                if(!mounted) return;
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Libro eliminado", style: TextStyle(color: Colors.white)), backgroundColor: Colors.green));
              } catch (e) {
